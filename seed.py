...
def rand_email():
    return f"user_{rands()}@example.com"
...
def run(users_n: int, notes_n: int, cats_n: int, reset: bool, fresh: bool):
    ...
    # 3) Категории
    if Category.objects.count() < cats_n:
        to_create = []
        for i in range(cats_n - Category.objects.count()):
            idx = i + 1 + Category.objects.count()
    ...
    # 4) Пользователи + профили (докидываем до нужного количества)
    have = User.objects.count()
    to_make = max(0, users_n - have)
    for _ in range(to_make):
        u = User.objects.create(name=rand_name(), email=rand_email())
    ...
    if not users:
        u = User.objects.create(name="Иван Иванов", email=rand_email())
    ...
    for _ in range(notes_n):
        ...
        n = Note.objects.create(author=author, status=status, text=rand_text(), created_at=created_at)
        n.categories.set(random.sample(categories, k=random.randint(1, min(3, len(categories)))))
